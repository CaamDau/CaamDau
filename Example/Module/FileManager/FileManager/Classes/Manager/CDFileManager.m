...
+(NSString*)nameForPath:(NSString*)path {
    NSString * utf = path.stringByRemovingPercentEncoding;
    NSString * name = [utf lastPathComponent];
    return name;
}

+(NSString*)nameForURL:(NSURL*)url {
    return [self nameForPath:[url absoluteString]];
}

+(NSString*)extensionForURL:(NSURL*)url {
    return [[self nameForURL:url] pathExtension];
}
+(NSString*)extensionForPath:(NSString*)path {
    return [[self nameForPath:path] pathExtension];
}
+(NSString*)extensionForName:(NSString*)name {
    return [name pathExtension];
}


+(BOOL)moveItemAtURL:(NSURL *)url toPath:(NSString *)toPath {
    NSString * name = [[self nameForURL:url] stringByDeletingPathExtension];
    NSString * exten = [self extensionForURL:url];
    NSString * theName = [name stringByAppendingPathExtension:exten];
    NSString * savePath = [toPath stringByAppendingPathComponent:theName];
    
    int suffix = 0;
    while ([self existsItemAtPath:savePath]) {
        suffix += 1;
        NSString * nowName = [name stringByAppendingString:[NSString stringWithFormat:@"-%d",suffix]];
        theName = [nowName stringByAppendingPathExtension:exten];
        savePath = [toPath stringByAppendingPathComponent:theName];
    }
    if ([[NSFileManager defaultManager] moveItemAtURL:url toURL:[NSURL fileURLWithPath:savePath] error:nil]) {
        return YES;
    }
    else{
        return NO;
...
{
    return [[CDFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[CDFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[CDFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[CDFileManager pathForLibraryDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[CDFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[CDFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
